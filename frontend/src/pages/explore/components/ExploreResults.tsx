...
export function ExploreResults({ results, scope, onOpenMember, onOpenPost, onOpenJob, onOpenProduct }: ExploreResultsProps) {
  ...
  return <div className="explore-result-groups">
    ...
    {groups.filter((group) => group.items.length).map((group) => <section key={group.title}><h2>{group.title}</h2><div className="explore-result-grid">{group.items.map((item) => <Button key={item.id} className="explore-result-card" variant="ghost" onClick={item.onOpen} disabled={!item.onOpen}><strong>{item.title}</strong><span>{item.description}</span>{"detail" in item && item.detail ? <small>{String(item.detail)}</small> : null}</Button>)}</div></section>)}
  </div>;
}
