...
import { replacePostImage } from "./api/replacePostImage";
...
export function PostEditorDialog({ open, post, onClose, onPostSaved }: PostEditorDialogProps) {
  const [content, setContent] = useState(""); const [category, setCategory] = useState("General Wellness"); const [mediaFile, setMediaFile] = useState<File | null>(null); const [removeMedia, setRemoveMedia] = useState(false); const [saving, setSaving] = useState(false); const [error, setError] = useState(""); const [notice, setNotice] = useState("");
  useEffect(() => { if (!open) return; setContent(post?.content ?? ""); setCategory(post?.category ?? "General Wellness"); setMediaFile(null); setRemoveMedia(false); setError(""); setNotice(""); }, [open, post]);
  if (!open) return null;
  async function savePost(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(""); setNotice(""); const existingMediaRemains = Boolean(post?.media_url && !removeMedia); if (!content.trim() && !mediaFile && !existingMediaRemains) { setError("Add text or media before saving the post."); return; } setSaving(true); try { const result = post ? mediaFile ? await replacePostImage(post.id, content, category, mediaFile) : await updatePost(post.id, { content, category, removeMedia }) : await publishPost({ content, category, mediaFile }); onPostSaved(result); setNotice(result.warning ?? (post ? "The server saved the post changes." : "The server published the post.")); } catch (saveError) { setError(describePostSaveFailure(saveError)); } finally { setSaving(false); } }
  return <div className="post-editor-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="post-editor-dialog" role="dialog" aria-modal="true" aria-labelledby="post-editor-title"><header><div><p>{post ? "Edit post" : "Create post"}</p><h2 id="post-editor-title">{post ? "Update your post" : "Share with Jorniz"}</h2></div><button type="button" onClick={onClose} aria-label="Close post editor">Close</button></header><form onSubmit={savePost}><label><span>Post text</span><textarea value={content} onChange={(event) => setContent(event.target.value)} placeholder="Share a healthcare insight" rows={7} /></label><label><span>Category</span><select value={category} onChange={(event) => setCategory(event.target.value)}>{postCategories.map((value) => <option key={value}>{value}</option>)}</select></label><PostMediaPicker selectedFile={mediaFile} existingMediaUrl={post?.media_url} existingMediaType={post?.media_type} removeExistingMedia={removeMedia} onFileChange={setMediaFile} onRemoveExistingMediaChange={setRemoveMedia} />{error ? <p className="post-editor-message post-editor-error">{error}</p> : null}{notice ? <p className="post-editor-message post-editor-notice">{notice}</p> : null}<footer><button type="button" onClick={onClose}>Cancel</button><button type="submit" disabled={saving}>{saving ? "Saving..." : post ? "Save changes" : "Publish post"}</button></footer></form></section></div>;
}
